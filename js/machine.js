  ...
  Machine.prototype = {
    ...
    prepareModels : function() {
      ...
      this.models.gantry.top = new THREE.Mesh(
        new THREE.CubeGeometry(
          0,
          0 + this.bounds.y.upper - this.bounds.y.lower,
          0
        ),
        this.basicMaterial
      );
      ...
      this.models.spindle.tool = new THREE.Mesh(
        new THREE.CylinderGeometry(4, 1, 80, 80, true),
        this.activeMaterial
      );
    ...
    processLine : function(gcode, done) {
      ...
      //3.每換行一次，形成一次陣列list[]
      //4.抓出Gcode的前面的英文代碼
      //5.switch case 判斷代碼狀態
      //6.加入時間變化量,每執行一次就push到lines的陣列。
      //7.使用three js 的 line 材質，須把geometry座標及line材質，加入lines陣列

      //g為全局,所有的-都替換
      //replace(/[ \t]/g) / 界定範圍 /
      //[ \t] []為list，\t為對齊
      //match，a到z的字母
      //ig case insensitive，不區分大小寫
      //subcommand index case 1
                 ...
                 that.timer = setTimeout(movementTick, 16);
                  ...
                  if (oz > that.settings.zCut && that.position.z > that.settings.zCut) {
                    ...
                    var line = new THREE.Line(
                      geometry,
                      new THREE.LineBasicMaterial( { color: 0x0057FF, linewidth: 1, opacity: 1 } ),
                      THREE.LinePieces
                    );
